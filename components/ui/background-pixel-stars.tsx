...
export type BackgroundPixelStarsProps = {
  /** Override positioning  defaults to a fixed full-viewport layer. */
  className?: string;
};

export const BackgroundPixelStars = memo(
  function BackgroundPixelStars({ className }: BackgroundPixelStarsProps = {}) {
    ...
    // Size the canvas to its parent so it can live inside a hero,
    // falling back to the viewport for full-page usage.
    ...
    useEffect(() => {
      ...
      // Reset global alpha (twinkle loop leaves it mutated)
      const initCtx = canvasRef.current.getContext("2d");
      if (initCtx) initCtx.globalAlpha = 1;
      ...
      const createShootingStar = (): void => {
        ...
        timeoutRef.current = setTimeout(createShootingStar, randomDelay);
      };
    ...
    }, [animateCanvas, createNewShootingStar, initBackgroundStars, regenerateBackgroundStars, measure]);
